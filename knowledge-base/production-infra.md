...
- **Sentry smoke shortcuts:** Installed builds expose hidden diagnostics shortcuts for release QA. `Ctrl+Alt+Shift+J` throws a JS error from `app/src/lib/error-toast.ts` so source-map frame resolution can be checked. `Ctrl+Alt+Shift+N` invokes a native Tauri command that panics with `sentry-native-stack-smoke-test` so app binary/PDB symbolication can be checked. In DevTools the same hooks are available as `window.__HOUSTON_SENTRY_SMOKE__.javascript()` and `window.__HOUSTON_SENTRY_SMOKE__.native()`. These are intentionally not visible in UI. Note: the native smoke panics the **app** process — there is no dedicated engine-process smoke trigger yet, so verify engine (`runtime=engine`) symbolication against a real engine crash or by adding a temporary panic to a route handler.
